...
function requireMailAdmin(actor: Actor) {
  if (actor.role !== 'system_admin') throw new ForbiddenException('仅平台管理员可维护邮件设置。');
}
...
export class MailConfigService {
  ...
  async read(actor: Actor) {
    ...
    if (!row) return { enabled: false, host: '', port: 465, security: 'tls', username: '', fromAddress: '', fromName: '元引统一认证', revision: 0, hasPassword: false };
  ...
  async test(to: string, actor: Actor) {
    ...
    await this.send(to, '邮件发送测试', '这是一封元引统一认证测试邮件，无需回复。');
