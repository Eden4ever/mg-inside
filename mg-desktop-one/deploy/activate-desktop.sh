...
for file in /opt/mg-desktop/secrets/desktop.env /opt/mg-desktop/secrets/files.env; do
  test -f "$file"
...
for image in mg-desktop-service mg-files-service; do docker image inspect "$image:$release_id" >/dev/null; done
backup_dir="/opt/mg-desktop/backups/$release_id"
test ! -e "$backup_dir"
install -d -m 0700 "$backup_dir"
if test -L /opt/mg-desktop/current; then readlink -f /opt/mg-desktop/current > "$backup_dir/previous-release"; fi
