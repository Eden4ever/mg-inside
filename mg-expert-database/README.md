...
面向营商环境考核指标的结构化知识库。保存即记录，不设提交审核、退回或发布流程；历史版本也按编辑权限继续维护。当前完成 M0-M3：领域契约、指标体系与三级指标树、八模块研究工作台、依据材料、不可变修订和乐观锁。AI 助手已从本应用移除。系统采用服务端会话，支持本地账号、用户与角色管理，并预留企业微信 `CorpApp` 扫码登录。

本应用不提供 AI 助手：工作台的 AI 指标专家面板、AI 建议接口、`ai_service` 角色和 DeepSeek 模型网关均已删除。语义库的向量检索及其智谱 embedding 配置仍保留，属于检索能力。研发期使用的 GPT、Kimi、MiniMax CLI 不属于产品运行时。
...
阶段一工程化的路由契约、测试证据和发布方式见 `docs/verification-stage1-engineering.md`（其中的 AI 采纳事务边界记录属于该功能移除前的历史）。
