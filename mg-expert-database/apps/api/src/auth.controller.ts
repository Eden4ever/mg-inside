...
import { actorFromRequest, AuthService, type AuthenticatedRequest, Public, SESSION_COOKIE, CHALLENGE_COOKIE, CHALLENGE_BROWSER_COOKIE } from './auth';
...
export class AuthController {
  ...
  async logout(@Req() req: AuthenticatedRequest, @Res({ passthrough: true }) reply: FastifyReplyLike): Promise<void> {
    if (req.authSession) await this.auth.logout(req.authSession.id, req.headers.authorization);
