...
export const WECOM_STATE_COOKIE = 'mg_expert_wecom_state';
...
export class WeComAuthService implements OnModuleInit, OnModuleDestroy {
  ...
  async complete(code: string, state: string, cookieHeader: unknown, context: { userAgent?: string; ipAddress?: string }) {
    ...
    const session = await this.auth.beginAuthentication(identity.user as User, context, 'wecom', transaction);
  ...
  private config(required: boolean): WeComConfig | null {
    if (process.env.IDENTITY_ENABLED === 'true') {
      if (required) throw new ConflictException('企业微信认证与人员同步已由统一身份服务管理。');
      return null;
    }
    const values = {
    ...
    if (required) throw new ConflictException('企业微信扫码登录尚未配置，请联系系统管理员。');
    return null;
  }
  ...
  private async performDirectorySync(actor?: Actor): Promise<WeComSyncResult> {
    ...
    const counts = await this.prisma.$transaction(async (transaction) => {
      ...
      for (const member of members) {
        ...
        if (identity) {
          const changed = identity.user.displayName !== displayName || identity.user.departmentName !== departmentName;
          if (changed) {
            await transaction.user.update({ where: { id: identity.userId }, data: { displayName, departmentName } });
            result.updated += 1;
          } else {
            result.unchanged += 1;
          }
        ...
        const normalizedUserId = externalUserId.toLowerCase();
        const canMatchUsername = /^[a-z0-9._-]{3,50}$/.test(normalizedUserId);
        const matchedUser = canMatchUsername ? await transaction.user.findUnique({ where: { username: normalizedUserId } }) : null;
        if (matchedUser) {
          // 跨身份源同名不代表同一人，只允许管理员显式绑定。
          result.conflicts += 1;
          continue;
        }

        const user = await transaction.user.create({ data: { username: null, displayName, passwordHash: null, role: 'reader', status: 'active', authSource: 'wecom', departmentName } });
        await transaction.weComIdentity.create({ data: { userId: user.id, corpId: config.corpId, externalUserId } });
        result.created += 1;
      }
  ...
  private directorySyncEnabled(): boolean {
    return process.env.IDENTITY_ENABLED !== 'true' && process.env.WECOM_DIRECTORY_SYNC_ENABLED === 'true';
  }
