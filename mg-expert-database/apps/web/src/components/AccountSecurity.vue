...
const action = ref<'bind' | 'remove' | 'email-bind' | 'email-remove' | 'key-bind' | 'key-remove' | 'enable' | 'disable' | null>(null);
const keyName = ref(''); const removingKey = ref('');
...
async function execute() {
  ...
  try {
    const proof = state.value.mfaEnabled && credentials.method === 'key' ? await startAuthentication({ optionsJSON: await api.securityKeyOptions() }) : credentials.code;
    ...
    else if (action.value === 'key-bind') {
      const started = await api.startKeyBinding(token, keyName.value.trim());
      const response = await startRegistration({ optionsJSON: started.options });
      if (!(await api.confirmKeyBinding(started.token, response)).ok) throw new Error('安全密钥验证失败，请重新绑定');
      keyName.value = ''; ElMessage.success('安全密钥已绑定');
    } else if (action.value === 'key-remove') { await api.removeKeyBinding(token, removingKey.value); ElMessage.success('安全密钥已移除'); }
    else {
...
<template>
  <section class="security-options" aria-label="多因素验证设置">
    ...
    <template v-if="state">
      ...
      <div class="security-row"><div><h3>安全密钥</h3><p>支持安全密钥和设备通行密钥</p></div><el-button :disabled="busy || state.keys.length >= 10" @click="open('key-bind')">添加密钥</el-button></div>
      <div v-for="key in state.keys" :key="key.id" class="security-row"><span>{{ key.name }}</span><el-button :disabled="busy" @click="removingKey = key.id; open('key-remove')">移除</el-button></div>
    ...
    <el-dialog :model-value="Boolean(action)" title="验证身份" width="440px" :close-on-click-modal="false" :close-on-press-escape="!busy" :show-close="!busy" @close="action = null; credentials.password = ''; credentials.code = ''">
      <el-form label-position="top" @submit.prevent="execute">
        ...
        <el-form-item v-if="action === 'key-bind'" label="密钥名称"><el-input v-model="keyName" maxlength="100" placeholder="例如：办公电脑" :disabled="busy" /></el-form-item>
      ...
      <el-alert v-if="error" :title="error" type="error" :closable="false" />
      <template #footer><el-button :disabled="busy" @click="action = null">取消</el-button><el-button type="primary" :loading="busy" :disabled="!credentials.password" @click="execute">确认</el-button></template>
    </el-dialog>
