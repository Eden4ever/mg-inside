...
const form = ref<MailSettings>({ enabled: false, host: '', port: 465, security: 'tls', username: '', fromAddress: '', fromName: '营商环境指标知识库', revision: 0, hasPassword: false });
...
watch([dirty, loaded, busy], () => emit('closeStateChange', { dirty: loaded.value && dirty.value, busy: busy.value }), { immediate: true, flush: 'sync' });
...
<template>
  <main class="primary-page">
    <header class="primary-page-heading"><h1>邮件设置</h1></header>
