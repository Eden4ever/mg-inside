...
assert re.fullmatch(r'stage1[5-9]-\d{8}-\d{6}', release_id)
...
for name, digest in manifest['files'].items():
    assert name in {'catalog.service.ts', 'catalog.service.js', 'verify-summary-hotfix.cjs'}
...
for source, destination in [
    ...
    ('verify-summary-hotfix.cjs', 'verify-summary-hotfix.cjs'),
]:
...
run(str(node), '--env-file=/etc/mg-expert-database/api.env', str(release / 'verify-summary-hotfix.cjs'), str(release), cwd=release)
(release / 'summary-hotfix-release.json').write_text(json.dumps({'release': release_id, 'previous': str(previous), **manifest}, indent=2) + '\n')
switch(release)
try:
