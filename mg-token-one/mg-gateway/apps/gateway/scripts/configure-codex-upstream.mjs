// This script is deliberately narrow: it only replaces CCTQ-Codex ciphertext.
...
export async function configureCodexCredential({ db, codexKey, jwtSecret }) {
  const key = requireNonBlank('CCTQ_CODEX_KEY', codexKey)
  ...
  try {
    ...
    if (channels.length === 0) {
      throw new Error('生产数据库缺少 CCTQ-Codex 渠道；拒绝创建伪渠道')
    }
    if (channels.length !== 1) throw new Error('CCTQ-Codex 渠道不唯一，拒绝更新凭据')
    ...
    if (updated.affectedRows !== 1) {
      throw new Error('CCTQ-Codex 凭据更新未命中唯一渠道')
    }
    await db.commit()
...
async function main() {
  const codexKey = requireNonBlank('CCTQ_CODEX_KEY', process.env.CCTQ_CODEX_KEY)
  ...
  try {
    const channelId = await configureCodexCredential({ db, codexKey, jwtSecret })
    console.log(JSON.stringify({ channel: 'CCTQ-Codex', channelId, credentialUpdated: true }))
  } finally {
    await db.end()
  }
}
